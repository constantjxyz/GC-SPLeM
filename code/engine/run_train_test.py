...
from engine.train_test_survival import train_survival, test_survival
from engine.train_test_path_survival import train_path_survival, test_path_survival
from dataset.create_dataset import PatientDataset, MtmcatDataset
from model.model_mtMCAT import MTMCAT
from engine.loss import mtmcat_loss
...
def run_train_test(train_patient, val_patient, test_patient, params=dict()):
    ...
    if params['perslide'] == 'True':
        train_tensor_dataset = PerslideDataset(train_patient, params=params, isTrain=True)
        val_tensor_dataset = PerslideDataset(val_patient, params=params)
        test_tensor_dataset = PerslideDataset(test_patient, params=params)
        # if params['checkSplit'] == 'True':
        #     print(train_tensor_dataset.pathology_file['case_id'].tolist())
        #     print(val_tensor_dataset.pathology_file['case_id'].tolist())
        #     print(test_tensor_dataset.pathology_file['case_id'].tolist())
        #     exit(0)
    # print(train_tensor_dataset.gene_file)
    else:
        train_tensor_dataset = MtmcatDataset(train_patient, params=params, isTrain=True)
        val_tensor_dataset = MtmcatDataset(val_patient, params=params)
        test_tensor_dataset = MtmcatDataset(test_patient, params=params)

    # complement the parameters in params dictionary
    data = train_tensor_dataset.__getitem__(0)
    ...
    task = params['task'] 
    params['task_num'] = int(train_tensor_dataset.task_num)
    params['task_name'] = train_tensor_dataset.clinical_columns_list
